...
class AKTMODEL(nn.Module):
    ...
    def forward(self, q_data, qa_data, target, t=-1, pid_data=None, annotated_qa=None):
        batch_size = q_data.shape[0]
        seqlen = q_data.shape[1]
        
        # Embedding calculations (unchanged)
        ...
        # PID embedding calculations (unchanged)
        if self.n_pid > 0:
        ...
        if t == -1:
            ...
            mask = torch.zeros_like(target_1d, dtype=torch.bool)
            mask[t::seqlen] = True
            mask = mask & target_1d.ge(0)

        # Apply mask
        filtered_pred = torch.masked_select(pred, mask)
        ...
        if annotated_qa is not None:
            annotated_output = []
            type_num = annotated_qa.max().item()
            for i in range(1, type_num + 1):
                annotated_mask = (annotated_qa == i).view(-1) & mask
                annotated_output.append([
                    torch.sigmoid(torch.masked_select(pred, annotated_mask)).detach().cpu(),
                    torch.masked_select(target_1d, annotated_mask).detach().cpu()
                ])
