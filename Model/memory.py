...
class DKVMNHeadGroup(nn.Module):
    ...
    def write(self, control_input, memory, write_weight=None):
        ...
        # print("head_control_input:", control_input.shape)
        # print("self.erase:", self.erase)
        # print("self.erase(control_input):", self.erase(control_input).shape)
        # qaqa = torch.cat([control_input, control_input], 1)
        # cuda0 = torch.device('cuda:0')
        # zeros = torch.zeros(control_input.shape)
        # zeros = zeros.to(cuda0)
        # qaqa = torch.cat([control_input, zeros], 1)
        # print("self.erase(control_input*2)", self.erase(qaqa).shape)
        # print("self.erase(control_input*2)", (self.erase(qaqa) == self.erase(control_input)).sum())
...
class DKVMN(nn.Module):
    ...
    def read(self, read_weight):
        #print('read',self.memory_value.shape)
