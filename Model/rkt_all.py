...
def attention(query, key, value, rel, l1, l2, timestamp, mask=None, dropout=None):
    ...
    # print(prob_attn.shape)
    # print(rel_attn.shape)
...
class RKTMODEL(nn.Module):
    ...
    def init_embeddings(self):
        ...
        nn.init.kaiming_normal_(self.item_embeds.weight)
        nn.init.kaiming_normal_(self.pos_key_embeds.weight)
        nn.init.kaiming_normal_(self.pos_value_embeds.weight)

    def forward(self, item_inputs, label_inputs, item_ids, rel, timestamp,label,t = -1, annotated_qa = None):
        batch_size = item_inputs.shape[0]
        seqlen = item_inputs.shape[1]
        ...
        att_output = outputs[:, :-1]
        zeros_padding = torch.zeros(att_output[:, 0].shape).cuda()
        outputs = torch.cat([zeros_padding.unsqueeze(1), att_output], 1)
        outputs = self.dropout(outputs)
        # outputs = self.sigmoid(outputs.squeeze(-1))
        outputs = self.lin_out(outputs)

        labels = label.reshape(-1)
        preds = (outputs.reshape(-1))  # logit
        ...
        if t == -1:
            ...
            mask_first = mask_first.reshape(-1)
            mask = (labels.ge(0) * mask_first).bool()  # [batch_size * seq_len, 1]
        # print(preds.shape)
        # mask = labels > -0.9
        ...
        if annotated_qa is not None:
            ...
            for i in range(1, type_num + 1):
                annotated_output.append([torch.sigmoid(torch.masked_select(preds, (annotated_qa == i).view(-1) & mask)).detach().cpu(),
                                       torch.masked_select(labels, (annotated_qa == i).view(-1) & mask).detach().cpu()])

            return loss, torch.sigmoid(masked_preds), masked_labels, torch.sigmoid(outputs).reshape(batch_size, seqlen), annotated_output
        else:
            return loss, masked_preds, masked_labels, torch.sigmoid(outputs).reshape(batch_size, seqlen)
